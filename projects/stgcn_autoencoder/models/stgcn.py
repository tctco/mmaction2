...
class unit_social(BaseModule):
    def __init__(self, num_persons, channel):
        num_kpts = 8
        self.residual = True
        super().__init__(
...
class STGCNAnimal(BaseModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.in_channels != self.base_channels:
            ...
            if num_person > 1 and enable_social:
                modules.append(unit_social(num_person, base_channels))
        ...
        for i in range(2, num_stages + 1):
            ...
            modules.append(
                STGCNBlock(
                    in_channels, out_channels, A.clone(), stride, **lw_kwargs[i - 1]
                )
            )
            if num_person > 1 and enable_social:
                modules.append(unit_social(num_person, out_channels))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.num_person > 1 and self.enable_social:
